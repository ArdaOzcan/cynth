...
typedef struct
{
    ...
    uint32_t delta_frame_note_start;
    uint32_t delta_frame_note_end;
} Voice;
...
cynth_envelope_get_volume(const CynthEnvelope* envelope,
                          double delta_note_start_sec,
                          double delta_note_end_sec)
{
    double env = 0;
    double duration = delta_note_start_sec - delta_note_end_sec;
    ...
    if (delta_note_end_sec > 0) {
        /* Release */
        env *= 1 - (delta_note_end_sec / envelope->release);
    }
    if (env < 0)
        env = 0;
...
cynth_voice_note_start(Voice* voice, uint8_t note)
{
    ...
    voice->delta_frame_note_start = 0;
    voice->delta_frame_note_end = 0;
}
...
test(void)
{
    ...
    CynthEnvelope envelope = {
        .attack = 0.1f, .decay = 0.5f, .sustain = 0.25f, .release = 0.1f
    };
